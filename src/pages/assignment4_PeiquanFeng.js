...
//the values of the object are the value in the corresponding row in the csv file
...
const Charts = () => {
    ...
    const BAR_MARGIN = { top: 20, right: 100, bottom: 120, left: 60 };
    
    const innerBarWidth = BAR_WIDTH - BAR_MARGIN.left - BAR_MARGIN.right;
    const innerBarHeight = BAR_HEIGHT - BAR_MARGIN.top - BAR_MARGIN.bottom;
    
    const xScaleBar = d3.scaleBand()
        .domain(data.map(d => d.station))
        .range([0, data.length * 11])  // 每个站点给 15px 宽度（可调）
        .padding(0.2);
        
    const yScaleBar = d3.scaleLinear()
        .domain([0, 4000])  // 固定最大值为 4000，统一 Y 轴高度
